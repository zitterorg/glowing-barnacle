# @zitterorg/glowing-barnacle

![CI](https://github.com/zitterorg/glowing-barnacle/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@zitterorg/glowing-barnacle.svg?style=flat)](https://www.npmjs.com/package/@zitterorg/glowing-barnacle)
[![js-standard-style](https://img.shields.io/badge/code%20style-standard-brightgreen.svg?style=flat)](https://standardjs.com/)


`@zitterorg/glowing-barnacle` enables the use of [CORS](https://en.wikipedia.org/wiki/Cross-origin_resource_sharing) in a Fastify application.

## Install
```
npm i @zitterorg/glowing-barnacle
```
...
## Usage
Require `@zitterorg/glowing-barnacle` and register it as any other plugin, it will add an `onRequest` hook and a [wildcard options route](https://github.com/fastify/fastify/issues/326#issuecomment-411360862).
```js
import Fastify from 'fastify'
import cors from '@zitterorg/glowing-barnacle'
...
fastify.register(require('@zitterorg/glowing-barnacle'), (instance) => {
  return (req, callback) => {
    const corsOptions = {
...
By default, `@zitterorg/glowing-barnacle` adds a `onRequest` hook where the validation and header injection are executed. This can be customized by passing `hook` in the options. Valid values are `onRequest`, `preParsing`, `preValidation`, `preHandler`, `preSerialization`, and `onSend`.
...
fastify.register(require('@zitterorg/glowing-barnacle'), {
  hook: 'preHandler',
